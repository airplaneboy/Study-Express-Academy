        ...
        <div className='flex-1  border-gray-300 lg:px-10 md:pl-10 rounded-r-xl'>
          <ul role='list' className='divide-y divide-gray-200 '>
            {sidebarArray.map((headerItem) => (
              <li key={headerItem._id} id={headerItem.title} className='px-4 pb-4 sm:px-0'>
                <div className='bg-white overflow-hidden  border rounded-lg divide-y divide-gray-200'>
                  <div className='px-4 py-5 sm:px-6 font-bold text-gray-700 flex items-center gap-2 text-lg'>
                    ...
                    <NextLink href={`/subjects/${headerItem.title}`}>{headerItem.title}</NextLink>
                  </div>
                  ...
                  <div className='px-4 py-5 sm:p-6 bg-gray-50'>
                    <ul className=' grid grid-cols-2 gap-2'>
                      {headerItem.units.map((listItem) => (
                        <li key={listItem._id} className='text-gray-600 text-md hover:underline'>
                          <NextLink href={`/subjects/${listItem.title}`}>{listItem.title}</NextLink>
                        </li>
                      ))}
                    </ul>
                  </div>
