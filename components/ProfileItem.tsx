  ...
  return (
    <div className='w-full border-2 rounded-2xl p-5  mb-5 capitalize h-max flex-1'>
      ...
      <ul>
        {stats.map((stat) => {
          return (
            <li key={stat.id ? stat.id : stat.property} className='flex justify-between items-center mb-1'>
              <h5 className='text-gray-700 capitalize'>{stat.property}:</h5>
              <p
                className={`text-gray-500 truncate max-w-[10rem] sm:max-w-[15rem] lg:max-w-lg ${
