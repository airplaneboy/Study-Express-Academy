  ...
  social: [
    {
      ...
      href: '#',
      icon: (props: any) => (
        <svg fill='#0269e3' viewBox='0 0 24 24' {...props}>
          <path
            fillRule='evenodd'
    ...
    },
    {
      ...
      href: '#',
      icon: (props: any) => (
        <svg fill='#f42773' viewBox='0 0 24 24' {...props}>
          <path
            fillRule='evenodd'
    ...
    },
    {
      ...
      href: '#',
      icon: (props: any) => (
        <svg fill='#1b9aef' viewBox='0 0 24 24' {...props}>
        ...
        <div className='mt-2 sm:mt-auto w-full flex justify-evenly sm:justify-center sm:space-x-6 sm:py-2 rounded-3xl sm:w-max'>
          {navigation.social.map((item) => (
            <Link key={item.name} href={item.href} className='text-gray-400 hover:text-gray-500'>
              ...
              <item.icon className='md:h-6 md:w-6 h-6 w-6' aria-hidden='true' />
            </Link>
          ))}
        </div>
        <span className='mt-5 my-2 sm:my-auto text-center text-[9px] sm:text-xs md:text-sm text-gray-500 font-semibold pointer-events-none '>
          &copy; {process.env.YEAR} {process.env.TITLE}, Inc. All rights reserved.
        </span>
        <span className='mb-1 sm:my-auto text-gray-400 text-center text-[9px] sm:text-xs md:text-sm'>
          Icons by{' '}
